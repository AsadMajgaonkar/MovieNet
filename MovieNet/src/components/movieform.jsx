...
const MovieForm = ({ movies, genres }) => {
    const navigate = useNavigate()

    const getMovie =async () => {
        if (id != 'new') {
            let updateMovie = await movies.find(movie => movie._id == id)
            ...
            setMovie({
                ...
                genre: { refId: genre._id, name: genre.name },
        ...
        <div className='col-8 mx-auto'>
            <form>
                ...
                <div className="mb-3">
                    ...
                    <select className="form-select" onChange={(event) => setMovie({ ...movie, genre_ID: event.target.value })}>
                        {genres.map(g => <option
                            key={g._id}
                            value={g._id}
